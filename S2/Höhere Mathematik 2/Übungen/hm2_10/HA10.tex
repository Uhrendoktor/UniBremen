...
\begin{document}
	\begin{enumerate}
		...
		\begin{eqnn}
			...
			\eqn[][\Rightarrow]{\dot{y}(t)}{\sqrt{\frac{g}{l}}((a-c_2)e^{t\sqrt{\frac{g}{l}}}-c_2e^{-t\sqrt{\frac{g}{l}}})}
			\eqnspace
			\eqntext{Da die Kette am Anfang liegt, die Geschwindigkeit $\dot{y}$ zum Zeitpunkt $t$ demnach $0$ ist ($\dot{y}(0)=0$) folgt:}
			\eqnf{0}{\sqrt{\frac{g}{l}}(a-2c_2)}
			\eqn{c_2}{\frac{a}{2}}
			...
			\eqn[][\Rightarrow]{y(t)}{\frac{a}{2}\left(e^{t\sqrt{\frac{g}{l}}}+e^{-t\sqrt{\frac{g}{l}}}\right)}
			\eqntext{Zum Zeitpunkt des Abrutschen, muss $y(t)=l$ betragen (Die gesamte Kette ist vom Tisch gerutscht):}
			\geqnf{l}{\feq}{\frac{a}{2}\left(e^{t\sqrt{\frac{g}{l}}}+e^{-t\sqrt{\frac{g}{l}}}\right)}
			\eqn{l}{a\cosh(\sqrt{\frac{g}{l}}t)}
			\eqn{t}{\arccosh(\frac{l}{a})\sqrt{\frac{l}{g}}}
		\end{eqnn}
		
		\item[10.3] Löse die folgenden Anfangswertaufgaben:
		\begin{enumerate}
			\item $\ddot{y}+2\dot{y}-3y=4e^{2t}, y(0)=0, \dot{y}(0)=1$.
			\begin{eqnn}
				...
				\eqntext{da $b(t)=4e^{2t}$ mit $\lambda=2\neq\lambda_1,\lambda_2$ liegt kein Resonanzfall vor:}
				\eqn{y_s(t)}{4c_3e^{2t}}
				\eqn{\dot{y}_s(t)}{8c_3e^{2t}}
				\eqn{\ddot{y}_s(t)}{16c_3e^{2t}}
				\eqntext{in Ausgangsgleichung einsetzen:}
				\eqn[][\Rightarrow]{20c_3e^{2t}}{4e^{2t}}
				\eqn{c_3}{\frac{1}{5}}
				\eqntext{$c_3$ in $y_s$ einsetzen:}
				\eqn[][\Rightarrow]{y_s(t)}{\frac{4}{5}e^{2t}}
				\eqn[][\Rightarrow]{y(t)}{c_1e^t+c_2e^{-3t}+\frac{4}{5}e^{2t}}
				\eqn{\dot{y}(t)}{c_1e^t-3c_2e^{-3t}+\frac{8}{5}e^{2t}}
			\end{eqnn}
			\begin{eqnn}
				\eqntext{Anfangswertbedingungen einsetzen:}
				\eqn[$y(0)=0$][]{c_1+c_2}{-\frac{4}{5}}
				\eqn[$\dot{y}(0)=1$][]{c_1-3c_2}{-\frac{3}{5}}
				\eqn[][\Rightarrow]{\m{1&1\\1&-3}[\add[\cdot(-1)][+]{0}{1}]c}{\m{-\frac{4}{5}\\-\frac{3}{5}}}
				\eqn{\m{1&1\\0&-4}c}{\m{-\frac{4}{5}\\\frac{1}{5}}}
				\eqn[][\Rightarrow]{c_2}{-\frac{1}{20}}
				\eqn[][\Rightarrow]{c_1}{-\frac{3}{4}}
				\eqntext{$c_1,c_2$ in $y$ einsetzen:}
				\eqn[][\Rightarrow]{y(t)}{-\frac{3}{4}e^t-\frac{1}{20}e^{-3t}+\frac{4}{5}e^{2t}}
			\end{eqnn}
		...
		\begin{enumerate}
			...
			%\begin{eqnn}
				%\eqntext{Da $y_1(t)=\sin(t)$ und $y_2(t)=t$ als Lösungsbasis von $y$ folgt:}
				%\eqnf{y}{\sin(t)+t}
				%\eqn[][\Rightarrow]{\dot{y}}{\cos(t)+1}
				%\eqn[][\Rightarrow]{\ddot{y}}{-\sin(t)}
				%\eqntext{In Ausgangsgleichung einsetzen:}
				%\eqn[][\Rightarrow]{0}{-\sin(t)+a_1(t)(\cos(t)+1)+a_0(t)(\sin(t)+t)}
				%\eqn{0}{\sin(t)(a_0(t)-1)+\cos(t)a_1(t)+a_1(t)+a_0(t)}
				%\eqnspace
				%\eqntext{$f(t,a_0,a_1)$ wir konstruiert als Funktion mit $a_0, a_1$ als Konstanten, welche jeden möglichen Wert, den $a_0(t), a_1(t)$ annehmen könnten (inkl. der Zeitabhängigen Werte) annehmen. Sollte die Ausgangsgleichung erfüllt sein, müsste sich eine Gerade der t-abhängigen Lösungen der Ausgangsgleichung auf der $a_0,a_1$ Ebene bilden. Die einzige Triviallösung $a_1=0, a_0=\frac{\ddot{y}}{y}$ wir hierbei ausgeschlossen, da $y\in\R$ uns somit $a_0$ bei den Nullstellen von von $y$ undefiniert wäre, $a_0,a_1$ jedoch $\R\to\R$ erfüllen müssen.}
				%\eqnf{f(t,a_0,a_1)}{\sin(t)(a_0-1)+\cos(t)a_1+a_1+a_0\feq 0}
				%\eqnspace
				%\eqntext{Die einzig mögliche Lösung für $f=0$, liegt bei $t=a_1=a_0=0$ und ist abhängig von $t$ (ein einzelner Punkt). Somit kann die allgemeine Ausgangsgleichung unabhängig von $a_0(t),a_1(t)$ für die Lösungsbasis $y_1(t)=\sin(t), y_2(t)=t$ nicht erfüllt werden.}
			%\end{eqnn}
			%$\Rightarrow$ Es ist unmöglich mit der Lösungsbasis aus eine homogene lineare Differentialgleichung 2.Ordnung aufzustellen, selbst wenn man zeitabhängige Koeffizienten zulässt.			
